...
REPORT_ASSETS=('ETH','WETH','BNB','POL','AVAX','xDAI','MNT','S')
NATIVE_ASSETS={'ETH','BNB','POL','AVAX','xDAI','MNT','S'}
...
def check_evm(addresses,stop=None,log=lambda s:None):
 ...
 def scan(chain):
  ...
  items=[]
  if chain['symbol'] in NATIVE_ASSETS:items.append((chain['symbol'],'native','',18,chain.get('price_id')))
