...
class App(tk.Tk):
 def __init__(self):
  super().__init__();self.title('Wallet Audit V2 �?" 12-chain EVM checker');self.geometry('900x680');self.stop=threading.Event();self.events=queue.Queue();self.files=[];self.mapping=[];self.phrases=[];self.addresses=[]
  box=ttk.Frame(self,padding=14);box.pack(fill='both',expand=True)
  ttk.Label(box,text='Wallet Audit V2',font=('',16,'bold')).pack(anchor='w');ttk.Label(box,text='12 EVM networks, no Bitcoin or Fantom. Seed phrases stay local.').pack(anchor='w',pady=(0,8))
 ...
 def save(self):
  ...
  try:
   ...
   address_fields=['seed_id','address','kind','path','portfolio_usd','ETH','WETH','BNB','POL','AVAX','xDAI','MNT','S','coverage']
   write_csv(Path(folder)/'address_portfolio.csv',self.summary,address_fields)
   seed_fields=['seed_id','metamask_address_0','evm_addresses_checked','portfolio_usd','ETH','WETH','BNB','POL','AVAX','xDAI','MNT','S']
   if self.private.get():
   ...
   found_fields=['record_type','seed_id','metamask_address_0','wallet_address','portfolio_usd','ETH','WETH','BNB','POL','AVAX','xDAI','MNT','S']
   ...
   Path(folder,'README.txt').write_text('Wallet Audit V2 checks 12 EVM networks with no Bitcoin or Fantom. balances_found_PRIVATE.csv contains only seed phrases or listed wallets where at least one checked asset is 0.001 or greater; each seed phrase appears once. Assets: ETH, WETH, BNB, POL, AVAX, xDAI, MNT, S. Empty amount/status is an error, not zero. PRIVATE files contain recovery phrases; store them offline.\n',encoding='utf-8')
   # #region agent log
